...
- ✅ Commit messages describe the *change*, in the format below, and nothing else.
...
- Never log or transmit keystroke *content*. Event **counts** only. If a task seems to require key content, it is a misread task — ask.
...
| An env var | `packages/config/src/env.ts` (Zod schema) + `.env.example` — same commit |
...
### API
- Deny by default. Every controller has a guard. `@Public()` must be explicit and is code-reviewed.
- Authorization is checked against the *resource*, not just the role: an employee reading `/time-entries?userId=X` must have `X === session.userId`. A manager must own the team. Write the test for the 403 case, not just the 200 case.
...
3. No secrets, no `.env`, no `console.log`.
4. Sensitive new fields added to the Pino redact list.
5. Migration committed alongside any schema change.
6. **Commit message contains no AI attribution, no co-author trailer, no generated-by footer, and the author is the repo's configured git user.**
