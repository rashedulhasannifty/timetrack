...
- [ ] 2.3 Activity monitoring + daily rollups. **2.3a (server/worker) done:** `ActivityDailySummary` table + `rollup-daily` processor (previous-UTC-day aggregate → idempotent upsert) + net-new 00:15-UTC repeatable scheduler + `GET /activity-summaries` (self/manager/admin via `@ResourceScope`). Minutes = sample count × `ACTIVITY_SAMPLE_INTERVAL_SECONDS`(60)/60; `windowTitle` never selected/logged. Live-driven: self=200, cross-user=403, unauth=401, bad-query=422; api functions coverage 86.58%. **Pending:** 2.3b client capture/categorization, 2.3c dashboard activity views.
