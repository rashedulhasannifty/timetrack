...
**Scope note (don't test the wrong thing):** the marker only helps under _partial_ connectivity — `AuthSession.bootstrap()` still needs the network to mint an access token, so a **fully cold-offline launch** hits the login window and is blocked (that's 1.7d territory). The 1.7b offline guarantee — "no live policy fetch per Start" — is satisfied by the session `isReady` flag alone; the marker is best-effort resilience for the auth-reachable / policy-unreachable window. Task 7's smoke step 7 (stop the API **mid-session**, then Start/Stop) is the correct offline test.
