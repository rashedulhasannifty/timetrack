...
Run: `RUN_E2E=1 pnpm --filter @timetrack/api exec vitest run test/storage.e2e-spec.ts`
...
Run: `RUN_E2E=1 pnpm --filter @timetrack/api exec vitest run test/screenshots.e2e-spec.ts`
...
- [ ] **Step 9: Run the processor integration test**

Run: `RUN_E2E=1 pnpm --filter @timetrack/worker exec vitest run test/screenshot-process.e2e-spec.ts`
Expected: PASS (3 blur-mode tests + gate check).

- [ ] **Step 10: Commit**

```bash
git add apps/worker/src/processors/screenshot-derive.ts apps/worker/src/processors/screenshot-derive.spec.ts apps/worker/src/infra/s3.provider.ts apps/worker/src/processors/screenshot-process.processor.ts apps/worker/src/worker.module.ts apps/worker/test/worker-harness.ts apps/worker/test/screenshot-process.e2e-spec.ts apps/worker/package.json pnpm-lock.yaml
...
Run: `RUN_E2E=1 pnpm --filter @timetrack/worker exec vitest run`
Expected: green (derive unit + processor integration).
