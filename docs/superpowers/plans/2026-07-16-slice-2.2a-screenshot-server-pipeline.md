...
> If `MinioContainer`'s method names differ in the installed version, use `getConnectionUrl()` / credentials per its typings; the container image is `minio/minio`. Verify with `node -e "console.log(Object.getOwnPropertyNames(require('@testcontainers/minio').MinioContainer.prototype))"` if the build complains.
...
```ts
  ...
  async upload(@Req() req: FastifyRequest, @CurrentUser() user: SessionUser): Promise<Screenshot> {
    ...
    return this.service.upload(part.file, meta, user);
  }
```

> `req.file()` comes from `@fastify/multipart`. `part.fields.<name>.value` holds the text field values; `part.file` is the file `Readable`. `UploadScreenshotMetaSchema.parse` throws a ZodError that the global problem+json filter renders as 422.
